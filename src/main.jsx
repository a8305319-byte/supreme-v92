 ...
 */
const SYSTEM_AUTH = {
  sovereign: { phone: '0976017938', key: '8305319' }, // 指揮官 (帳號/密碼由老闆指定)
...
const App = () => {
  ...
  const handleAddPersonnel = () => {
    ...
    const defaultPwd = '0' + newDriver.phone.slice(-4); // 預設密碼邏輯
    const driverObj = {
    ...
    logAction('人員建檔', `新增司機: ${newDriver.name}，預設金鑰為 ${defaultPwd}`);
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      insurance_sub: Math.round(labor_employer + nhi_employer).toLocaleString(),
  ...
  const staffProfiles = {
    aide: { title: "指揮官特助", icon: UserCog, color: "var(--neon-cyan)", greeting: `長官好，數據已彙整完畢。目前隊列共有 ${drivers.length} 名司機。`, actions: ["彙整經營週報", "同步防禦環境"] },
    recruiter: { title: "招募專員", icon: Briefcase, color: "var(--neon-emerald)", greeting: "長官好，正在進行人員資料庫維修。建檔模組運作正常。", actions: ["新增司機建檔"] },
    dispatcher: { title: "調度專員", icon: Headset, color: "var(--neon-amber)", greeting: `你好 ${activeDriver.name}，路上平安。有任何狀況隨時回報。`, actions: ["回報車輛異常", "忘記登入金鑰"] },
    engineer: { title: "IT 工程師", icon: Monitor, color: "var(--neon-cyan)", greeting: "數據鏈路穩定。權限防禦盾已開啟，已鎖定主權識別。 ", actions: ["導出日誌", "洗滌快取"] },
  ...
  const handleSendMessage = (text = chatInput) => {
    ...
    setTimeout(() => {
      ...
      if (text.includes("金鑰") || text.includes("忘記密碼") || text.includes("忘記帳號")) {
        // AI 自動識別帳號並重置密碼邏輯 (需求 4)
        reply = `好的，這邊核對身分無誤。已將您的安全金鑰重置為預設格式：0+手機後四碼。請重新登入並自行修改。`;
        logAction('重置金鑰', `後勤為手機尾數 ${activeDriver.phone.slice(-4)} 司機恢復預設金鑰`);
      } else if (staffRole === 'dispatcher') {
        reply = `收到！已幫你立案並聯絡調度中心處理。請注意行車安全。`;
      } else {
  ...
  const handleLogin = () => {
    if (loginInput.phone === SYSTEM_AUTH.sovereign.phone && loginInput.pwd === SYSTEM_AUTH.sovereign.key) {
      ...
      } else alert("識別碼或金鑰錯誤。如遺失帳密，請諮詢通訊視窗。");
    }
  };
    ...
    <GlassCard className={`p-8 border-cyan-500/20 mb-6 transition-all ${isAddingPersonnel ? 'h-auto' : 'h-20 overflow-hidden'}`}>
       ...
       {isAddingPersonnel && (
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 animate-in slide-in-from-top-4 duration-300">
             ...
             <input placeholder="手機號碼 (做為帳號)" value={newDriver.phone} onChange={e => setNewDriver({...newDriver, phone: e.target.value})} className="manual-input !text-sm !text-left" />
             ...
             <div className="flex items-center gap-3 px-4 bg-white/5 rounded-xl border border-white/10">
                ...
                <span className="text-[10px] font-black text-white/40 uppercase">啟動保險精算</span>
             </div>
  ...
  const renderSovereign = () => (
    <div className="space-y-6 animate-in fade-in duration-700 pb-32 pt-6 px-4">
      <div className="flex justify-between items-center text-rose-500 uppercase tracking-tighter">
        <h2 className="text-2xl font-black italic">主權決策中心</h2>
        <div className="flex gap-2">
           <button onClick={() => handleMatrixToggle('kill_switch')} className={`btn-tactile px-4 py-2 rounded-xl text-[10px] font-black uppercase ${matrix.kill_switch ? 'bg-rose-600 shadow-[0_0_15px_rgba(244,63,94,0.4)]' : 'bg-white/5 border border-white/10 text-white/40'}`}>
             <Zap size={14} /> {matrix.kill_switch ? '系統已熔斷' : '緊急營運熔斷'}
           </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="glass-card p-6 border-l-4 border-l-cyan-500/50">
          <p className="text-[10px] text-white/30 uppercase mb-1 font-mono tracking-widest">本週經營利潤</p>
        ...
        <div className={`glass-card p-6 border-l-4 ${metrics.isAnomaly ? 'alert-pulse border-l-rose-500' : 'border-l-amber-500/50'}`}>
          <p className="text-[10px] text-white/30 uppercase mb-1 font-mono tracking-widest">待查案件</p>
      ...
      <div className="glass-card p-8">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-6">
                <div className={`flex justify-between items-center p-4 rounded-2xl bg-black/40 border transition-all ${activeDriver.is_insured ? 'border-white/5' : 'border-rose-500/20 shadow-[0_0_10px_rgba(244,63,94,0.1)]'}`}>
                ...
                <div className={`grid grid-cols-2 gap-4 ${!activeDriver.is_insured ? 'opacity-20 pointer-events-none' : ''}`}>
                    ...
                    <div className="space-y-2"><label className="text-[9px] text-white/40 uppercase font-bold">眷屬人數</label><input type="number" value={activeDriver.person_count} onChange={e => updateDriverData(selectedDriverId, 'person_count', parseInt(e.target.value))} className="manual-input" /></div>
                </div>
            </div>
            <div className="grid grid-cols-2 gap-4 border-t lg:border-t-0 lg:border-l border-white/5 pt-6 lg:pt-0 lg:pl-8">
               ...
               <div className="space-y-2"><label className="text-[9px] text-white/40 uppercase font-bold">逆物流單價 ($/件)</label><input type="number" step="0.05" value={activeDriver.piece_unit_returned} onChange={e => updateDriverData(selectedDriverId, 'piece_unit_returned', parseFloat(e.target.value))} className="manual-input !text-orange-400" /></div>
            </div>
         </div>
      </div>
  ...
  const renderAdmin = () => (
    <div className="space-y-6 animate-in fade-in duration-700 pb-32 pt-6 px-4 text-white/80">
      <div className="flex justify-between items-center uppercase tracking-tighter">
        <h2 className="text-2xl font-black">稽核中樞</h2>
        <select value={selectedDriverId} onChange={e => setSelectedDriverId(e.target.value)} className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-xs font-black text-cyan-400 outline-none">
      ...
      <GlassCard className={`p-8 space-y-6 relative overflow-hidden transition-all ${metrics.isAnomaly ? 'border-rose-500/40' : ''}`}>
        {!matrix.admin_rate_edit && (
          <div className="absolute inset-0 z-10 bg-black/70 backdrop-blur-[6px] flex flex-col items-center justify-center p-8 text-center">
             ...
             <p className="text-rose-500 font-black uppercase text-xs tracking-widest leading-relaxed">權限已由指揮官鎖定<br/><span className="text-[9px] opacity-60">需開啟「授權會計修改設定」維度</span></p>
          </div>
        )}
        <div className="flex justify-between items-center border-b border-white/5 pb-4"><p className="text-[10px] font-black text-amber-400 uppercase tracking-widest flex items-center gap-2"><Calculator size={12}/> 精算對照明細</p>{metrics.isAnomaly && <span className="bg-rose-600 text-white px-2 py-0.5 rounded font-black animate-pulse uppercase text-[9px]">數值偏差警示: {metrics.deviation_percent}%</span>}</div>
        
        <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-6">
           <div className="p-4 rounded-xl bg-black/20 text-center"><p className="text-[9px] text-white/20 uppercase mb-2 font-mono">二代健保代扣</p><p className="text-xl font-mono text-rose-500/80 font-black">-${metrics.supplement}</p></div>
           <div className="p-4 rounded-xl bg-black/20 text-center"><p className="text-[9px] text-white/20 uppercase mb-2 font-mono">核定實領總額</p><p className="text-xl font-mono text-emerald-400/80 font-black">${metrics.real_payout}</p></div>
        </div>

        <div className="grid grid-cols-2 gap-4">
           <div className="p-5 rounded-xl bg-black/40 border border-white/5 text-center"><p className="text-[9px] text-white/30 uppercase mb-3 font-bold">配送數 (手動核定)</p><input type="number" value={activeDriver.delivered} onChange={e => updateDriverData(activeDriver.id, 'delivered', parseInt(e.target.value)||0)} className="manual-input" /></div>
           <div className="p-5 rounded-xl bg-black/40 border border-white/5 text-center"><p className="text-[9px] text-white/30 uppercase mb-3 font-bold">回收數 (手動核定)</p><input type="number" value={activeDriver.returned} onChange={e => updateDriverData(activeDriver.id, 'returned', parseInt(e.target.value)||0)} className="manual-input !text-orange-400" /></div>
        </div>
        
        <textarea placeholder="核銷理由或偏差備註 (此紀錄將同步至決策看板足跡)..." className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-xs h-20 outline-none resize-none focus:border-cyan-500/50 text-white/80 font-bold" />
        <button disabled={!activeDriver.has_photo || matrix.kill_switch || metrics.isAnomaly} className={`w-full py-6 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl btn-tactile ${(!activeDriver.has_photo || matrix.kill_switch || metrics.isAnomaly) ? 'bg-white/5 text-white/10' : 'bg-emerald-600 text-white'}`}>確認核銷並發放</button>
      </GlassCard>
    </div>
  );
  ...
  const renderSentinel = () => (
    <div className="space-y-6 animate-in fade-in duration-700 pt-6 pb-32 px-4">
      <div className="glass-card p-6 border-l-4 border-amber-500 shadow-amber-900/10 relative overflow-hidden">
        <div className="flex items-center gap-3 mb-3 text-amber-400"><Bell size={16} className="animate-bounce" /><span className="text-[10px] font-black uppercase tracking-widest">總部公告與行車安全</span></div>
        <p className="text-sm font-bold text-white/95 leading-relaxed italic relative z-10 font-mono">"{bulletin}"</p>
      </div>

      <div className="text-center pt-4 uppercase italic"><h2 className="text-2xl font-black tracking-widest text-white/90">哨兵數據終端</h2><p className="text-[10px] text-white/30 tracking-[0.4em] mt-2 font-mono">正在提交身分：{activeDriver.name} ({activeDriver.phone})</p></div>
      ...
      <div className="glass-card p-12 flex justify-around items-center">
        {matrix.empire_dial ? (
          ...
          <div className="py-12 text-center opacity-40 flex flex-col items-center"><ShieldAlert size={64} className="text-rose-500 mb-6 animate-pulse" /><p className="text-rose-500 text-xs font-black uppercase tracking-widest">權限已由中心鎖定</p></div>
        )}
      </div>

      <GlassCard className="p-6">
         <h3 className="text-[10px] font-black text-white/30 uppercase tracking-widest flex items-center gap-2 mb-4"><Key size={14}/> 終端安全性設定 (金鑰修改)</h3>
         <div className="flex gap-4">
            ...
            <button className="px-6 bg-cyan-600/20 border border-cyan-500/30 rounded-xl text-[10px] font-black text-cyan-400 uppercase tracking-widest transition-all active:scale-95">更新授權碼</button>
         </div>
      </GlassCard>

      <button className="w-full btn-tactile py-6 rounded-2xl bg-cyan-600 text-white font-black text-xs uppercase tracking-[0.5em] shadow-xl">完成本日數據提交 (COMMIT)</button>
    ...
    <div className="h-screen w-screen flex flex-col no-scrollbar relative font-sans overflow-hidden">
      ...
      {view === 'splash' && (
        <div className="h-screen w-screen bg-black flex flex-col items-center justify-center scanner-active overflow-hidden">
          ...
          <p className="text-[10px] text-cyan-500/50 mt-4 font-mono uppercase tracking-[0.3em]">企業資安盾 V3.0 防禦級啟動...</p>
        </div>
      )}
      ...
      {view === 'login' && (
        <div className="flex flex-col items-center justify-center h-screen px-8 space-y-12 animate-in fade-in duration-1000">
          ...
          <div className="w-full max-w-sm space-y-5">
            <input type="text" placeholder="手機號碼 / 指揮官識別碼" className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-sm text-white outline-none focus:border-cyan-500/50" onChange={e => setLoginInput({...loginInput, phone: e.target.value})} />
            <input type="password" placeholder="授權金鑰 / SECURITY KEY" className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-sm text-white outline-none focus:border-cyan-500/50" onChange={e => setLoginInput({...loginInput, pwd: e.target.value})} />
            ...
            <button onClick={handleLogin} className="w-full btn-tactile py-5 rounded-2xl bg-white text-black font-black uppercase tracking-widest shadow-xl">解鎖系統核心</button>
          </div>
        </div>
      )}
      ...
      {view === 'core' && (
        <React.Fragment>
          <header className="px-6 pt-12 pb-4 flex justify-between items-center z-50 bg-gradient-to-b from-black to-transparent">
            <div className="flex items-center gap-3"><Building2 size={18} className="text-cyan-400/60" /><span className="text-[10px] font-black tracking-widest uppercase text-white/40 italic">在地物流 v3.0 <span className="opacity-30 ml-2">82749921</span></span></div>
            <div className="flex items-center gap-4"><Fingerprint size={18} className={isVerified ? 'text-cyan-400 shadow-[0_0_10px_#22d3ee]' : 'text-white/10'} /><button onClick={() => {setView('login'); setIsVerified(false);}} className="text-white/20 hover:text-rose-500 transition-colors active:scale-95"><LogOut size={16}/></button></div>
          </header>
      ...
      {view === 'core' && (
        <React.Fragment>
          ...
          <div className={`fixed bottom-[110px] right-6 w-80 h-[520px] glass-card p-6 flex flex-col transition-all duration-500 origin-bottom-right z-[100] ${isChatOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-50 translate-y-20 pointer-events-none'}`}>
            ...
            <div className="mt-4 flex gap-2 shrink-0"><input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSendMessage()} placeholder="輸入通訊或指令..." className="flex-1 bg-white/5 border border-white/10 rounded-xl p-3 text-xs outline-none focus:border-cyan-500/50 text-white placeholder:text-white/20 font-bold" /><button onClick={() => handleSendMessage()} className="w-10 h-10 bg-cyan-500 rounded-xl flex items-center justify-center btn-tactile shadow-lg text-white active:scale-90"><Send size={14} /></button></div>
          </div>
        </React.Fragment>
      )}

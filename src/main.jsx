 ...
 */
const SYSTEM_AUTH = {
  sovereign: { phone: '0976017938', key: '07938' }, // 指揮官(老闆)
  admin: { phone: '0988128172', key: '08172' }      // 合規稽核(會計)
};

/**
 * === GPT PART 1: STYLES & THEME ===
 * 在地物流有限公司 - 核心美學與物理質感定義
 */
const GlobalStyles = () => (
...
const App = () => {
  ...
  const [bulletin, setBulletin] = useState("歡迎回到在地物流系統，請保持安全駕駛與精準核銷。");
  ...
  const handleAddPersonnel = () => {
    ...
    logAction('人員建檔', `${newDriver.name} 已錄入系統，初始金鑰: ${defaultPwd}`);
  ...
  const staffProfiles = {
    aide: { title: "指揮官特助", icon: UserCog, color: "var(--neon-cyan)", greeting: `長官好，今日司機隊列共 ${drivers.length} 人。`, actions: ["彙整營運週報", "防禦環境同步"] },
    recruiter: { title: "招募專員", icon: Briefcase, color: "var(--neon-emerald)", greeting: "正在維護人才庫。人員建檔模組已上線。", actions: ["新增司機建檔"] },
    dispatcher: { title: "調度專員", icon: Headset, color: "var(--neon-amber)", greeting: `你好 ${activeDriver.name}，路上平安。有狀況隨時回報。`, actions: ["回報車輛異常", "忘記登入金鑰"] },
    engineer: { title: "IT 工程師", icon: Monitor, color: "var(--neon-cyan)", greeting: "數據鏈路狀態穩定。權限防禦盾已開啟。", actions: ["導出操作日誌", "洗滌快取"] },
    butler: { title: "行政管家", icon: Ghost, color: "var(--neon-rose)", greeting: "指揮中心環境已優化。", actions: ["系統安全重置"] }
  };
  ...
  useEffect(() => {
    ...
    setTimeout(() => logAction('營運報告', '已自動生成本週經營損益分析報告'), 3800);
  ...
  useEffect(() => {
    ...
    if (isEraserHolding) {
      interval = setInterval(() => {
        setEraserProgress(p => {
          if (p >= 100) { setView('login'); logAction('緊急重置', '權限與會話已強制銷毀'); return 0; }
          return p + 4;
        });
      }, 100);
    ...
    <GlassCard className={`p-8 border-cyan-500/20 mb-6 transition-all ${isAddingPersonnel ? 'h-auto' : 'h-20 overflow-hidden'}`}>
       ...
       {isAddingPersonnel && (
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 animate-in slide-in-from-top-4 duration-300">
             <input placeholder="司機姓名" value={newDriver.name} onChange={e => setNewDriver({...newDriver, name: e.target.value})} className="manual-input !text-sm !text-left" />
             <input placeholder="手機號碼" value={newDriver.phone} onChange={e => setNewDriver({...newDriver, phone: e.target.value})} className="manual-input !text-sm !text-left" />
             <input placeholder="編號 (D-XXX)" value={newDriver.id} onChange={e => setNewDriver({...newDriver, id: e.target.value})} className="manual-input !text-sm !text-left" />
             <div className="flex items-center gap-3 px-4 bg-white/5 rounded-xl border border-white/10">
                ...
                <span className="text-[10px] font-black text-white/40 uppercase">勞健保計畫</span>
             </div>
  ...
  const renderSovereign = () => (
    <div className="space-y-6 animate-in fade-in duration-700 pb-32 pt-6 px-4">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-black italic text-rose-500 uppercase tracking-tighter">指揮決策中心</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="glass-card p-6 border-l-4 border-l-cyan-500/50 shadow-cyan-900/10">
          <p className="text-[10px] text-white/30 uppercase mb-1 font-mono tracking-widest">預估利潤率</p>
        ...
        <div className="glass-card p-6 border-l-4 border-l-emerald-500/50">
          <p className="text-[10px] text-white/30 uppercase mb-1 font-mono tracking-widest">在線司機員</p>
      ...
      <div className="glass-card p-8">
         <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4 text-[10px] font-black text-cyan-400 uppercase tracking-widest">
             <div className="flex items-center gap-2"><Users size={14}/> 司機人事與費率控制 (主權授權)</div>
            ...
            <div className="space-y-6">
                <div className={`flex justify-between items-center p-4 rounded-2xl bg-black/40 border transition-all ${activeDriver.is_insured ? 'border-white/5' : 'border-rose-500/20'}`}>
                   <div className="flex items-center gap-4">
                      ...
                      <div><p className="text-sm font-bold text-white/90">{activeDriver.name}</p><p className="text-[9px] text-white/20 uppercase tracking-widest">{activeDriver.is_insured ? '勞健保計畫已啟動' : '未投報外包模式'}</p></div>
                   </div>
                ...
                <div className={`grid grid-cols-2 gap-4 ${!activeDriver.is_insured ? 'opacity-20 pointer-events-none' : ''}`}>
                    <div className="space-y-2"><label className="text-[9px] text-white/40 uppercase font-bold">投保月額</label><input type="number" value={activeDriver.insurance_base} onChange={e => updateDriverData(selectedDriverId, 'insurance_base', parseInt(e.target.value))} className="manual-input" /></div>
      ...
      <div className="glass-card p-8">
         <h3 className="text-xs font-black text-cyan-400 uppercase tracking-widest mb-6 flex items-center gap-2 border-b border-white/5 pb-4"><Settings2 size={14}/> 系統維度權限防禦矩陣</h3>
         <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {Object.keys(matrix).filter(k=>k!=='kill_switch').map(key => (
              <div key={key} className="flex justify-between items-center p-3 rounded-2xl bg-black/40 border border-white/5">
                <span className="text-[10px] font-bold text-white/60 uppercase">{key === 'admin_rate_edit' ? '授權會計修改費率' : key.replace(/_/g,' ')}</span>
      ...
      <GlassCard className={`p-8 space-y-6 relative overflow-hidden transition-all ${metrics.isAnomaly ? 'border-rose-500/40' : ''}`}>
        {!matrix.admin_rate_edit && (
          <div className="absolute inset-0 z-10 bg-black/70 backdrop-blur-[6px] flex flex-col items-center justify-center p-8 text-center">
             ...
             <p className="text-rose-500 font-black uppercase text-xs tracking-widest leading-relaxed">權限已由指揮官鎖定<br/><span className="text-[9px] opacity-60">請洽管理層開啟「授權會計修改費率」</span></p>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-6">
           <div className="p-4 rounded-xl bg-black/20 text-center"><p className="text-[9px] text-white/20 uppercase mb-2 font-mono">代扣補充保費</p><p className="text-xl font-mono text-rose-500/80 font-black">-${metrics.supplement}</p></div>
           <div className="p-4 rounded-xl bg-black/20 text-center"><p className="text-[9px] text-white/20 uppercase mb-2 font-mono">核定實領金額</p><p className="text-xl font-mono text-emerald-400/80 font-black">${metrics.real_payout}</p></div>
        </div>

        <div className="grid grid-cols-2 gap-4">
           <div className="p-5 rounded-xl bg-black/40 border border-white/5 text-center"><p className="text-[9px] text-white/30 uppercase mb-3">核定配送數 (件)</p><input type="number" value={activeDriver.delivered} onChange={e => updateDriverData(activeDriver.id, 'delivered', parseInt(e.target.value)||0)} className="manual-input" /></div>
           <div className="p-5 rounded-xl bg-black/40 border border-white/5 text-center"><p className="text-[9px] text-white/30 uppercase mb-3">核定回收數 (件)</p><input type="number" value={activeDriver.returned} onChange={e => updateDriverData(activeDriver.id, 'returned', parseInt(e.target.value)||0)} className="manual-input !text-orange-400" /></div>
        </div>
        
        <textarea placeholder="核銷理由或偏差備註 (此紀錄將同步至主權足跡)..." className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-xs h-20 outline-none resize-none focus:border-cyan-500/50 text-white/80" />
  ...
  const renderSentinel = () => (
    <div className="space-y-6 animate-in fade-in duration-700 pt-6 pb-32 px-4">
      ...
      <div className="text-center pt-4"><h2 className="text-2xl font-black tracking-widest text-white/90 uppercase italic">哨兵終端結算</h2><p className="text-[10px] text-white/30 tracking-[0.4em] uppercase mt-2 font-mono">DRIVER: {activeDriver.name} ({activeDriver.phone})</p></div>
      ...
      <div className="glass-card p-12 flex justify-around items-center">
        {matrix.empire_dial ? (
          <div className="flex gap-4 sm:gap-12 text-center w-full justify-center">
             <EmpireDial value={activeDriver.delivered} label="配送完成件數" color="#22d3ee" onChange={(v) => updateDriverData(activeDriver.id, 'delivered', v)} />
             <EmpireDial value={activeDriver.returned} label="逆物流回收" color="#fbbf24" onChange={(v) => updateDriverData(activeDriver.id, 'returned', v)} />
          </div>
        ) : (
          <div className="py-12 text-center opacity-40 flex flex-col items-center"><ShieldAlert size={64} className="text-rose-500 mb-6 animate-pulse" /><p className="text-rose-500 text-xs font-black uppercase tracking-widest font-mono">權限由總部凍結</p></div>
        )}
      </div>

      <GlassCard className="p-6">
         <h3 className="text-[10px] font-black text-white/30 uppercase tracking-widest flex items-center gap-2 mb-4"><Key size={14}/> 終端安全設定</h3>
         <div className="flex gap-4">
            ...
            <button className="px-6 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black text-white/50 uppercase tracking-widest">更新</button>
         </div>
      </GlassCard>

      <button className="w-full btn-tactile py-6 rounded-2xl bg-cyan-600 text-white font-black text-xs uppercase tracking-[0.5em] shadow-xl">提交結算數據 (COMMIT)</button>
    ...
    <div className="h-screen w-screen flex flex-col no-scrollbar relative font-sans overflow-hidden">
      ...
      {view === 'login' && (
        <div className="flex flex-col items-center justify-center h-screen px-8 space-y-12 animate-in fade-in duration-1000">
          <div className="text-center">
             ...
             <p className="text-[10px] text-white/20 tracking-[0.3em] mt-2 font-mono uppercase">V3.0 Enterprise Secure Portal</p>
          </div>
          <div className="w-full max-w-sm space-y-5">
            ...
            <div className="flex items-center gap-3 px-1">
               ...
               <span className="text-[10px] font-black text-white/30 tracking-widest uppercase">記住我的帳號</span>
            </div>
            <button onClick={handleLogin} className="w-full btn-tactile py-5 rounded-2xl bg-white text-black font-black uppercase tracking-widest shadow-xl">啟動主權認證</button>
          </div>
        </div>
      )}
      ...
      {view === 'core' && (
        <React.Fragment>
          <header className="px-6 pt-12 pb-4 flex justify-between items-center z-50 bg-gradient-to-b from-black to-transparent">
            <div className="flex items-center gap-3"><Building2 size={18} className="text-cyan-400/60" /><span className="text-[10px] font-black tracking-widest uppercase text-white/40 italic">在地物流有限公司 <span className="opacity-30 ml-2 italic">82749921</span></span></div>
      ...
      {view === 'core' && (
        <React.Fragment>
          ...
          <div className={`fixed bottom-[110px] right-6 w-80 h-[520px] glass-card p-6 flex flex-col transition-all duration-500 origin-bottom-right z-[100] ${isChatOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-50 translate-y-20 pointer-events-none'}`}>
            <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4 shrink-0"><div className="flex items-center gap-3"><div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_#10b981]"></div><span className="text-xs font-black uppercase tracking-widest text-cyan-400">總部後勤連線</span></div><div className="flex gap-1">{Object.keys(staffProfiles).map(p => { const isLocked = role !== 'sovereign' && p !== staffRole; return <button key={p} disabled={isLocked} title={staffProfiles[p].title} onClick={() => setStaffRole(p)} className={`p-2 rounded-lg transition-all ${staffRole === p ? 'bg-white/15 text-white shadow-inner' : 'text-white/20 hover:bg-white/5'} ${isLocked ? 'opacity-0 scale-0' : ''}`}><UserCog size={14} /></button>})}</div></div>
            <div className="flex-1 overflow-y-auto no-scrollbar space-y-5 pr-1 flex flex-col"><div className="bg-white/5 p-4 rounded-2xl text-[11px] leading-relaxed text-white/70 border border-white/5 shadow-inner shrink-0"><p className="font-bold mb-2 flex items-center gap-2" style={{ color: staffProfiles[staffRole].color }}>[{staffProfiles[staffRole].title}]</p>{staffProfiles[staffRole].greeting}</div>{chatHistory.length === 0 && (<div className="grid grid-cols-1 gap-2 shrink-0">{staffProfiles[staffRole].actions.map(action => (<button key={action} onClick={() => handleSendMessage(action)} className="p-3 bg-black/40 border border-white/10 rounded-xl text-[10px] font-black text-white/50 hover:text-cyan-400 transition-all uppercase tracking-widest text-left pl-4 flex items-center gap-3 font-mono"><Zap size={10} /> {action}</button>))}</div>)}{chatHistory.map((msg, idx) => (<div key={idx} className={`p-3 rounded-2xl text-[11px] leading-relaxed max-w-[85%] shrink-0 shadow-lg ${msg.sender === 'user' ? 'bg-cyan-600/20 text-cyan-100 self-end ml-auto border border-cyan-500/20' : 'bg-white/5 text-white/70 border border-white/10 self-start'}`}>{msg.text}</div>))}<div ref={chatEndRef} className="shrink-0" /></div>
            <div className="mt-4 flex gap-2 shrink-0"><input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSendMessage()} placeholder="輸入通訊內容..." className="flex-1 bg-white/5 border border-white/10 rounded-xl p-3 text-xs outline-none focus:border-cyan-500/50 text-white placeholder:text-white/20 font-mono" /><button onClick={() => handleSendMessage()} className="w-10 h-10 bg-cyan-500 rounded-xl flex items-center justify-center btn-tactile shadow-lg text-white"><Send size={14} /></button></div>
          </div>
        </React.Fragment>
      )}
      ...
      {isBulletinOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/95 backdrop-blur-2xl animate-in fade-in duration-300">
           <div className="glass-card max-w-sm w-full p-8 space-y-6"><h3 className="text-xl font-black uppercase tracking-tighter flex items-center gap-3 text-amber-400"><Megaphone /> 廣播系統公告</h3><textarea placeholder="請輸入對司機端的最新指示..." value={tempBulletin} onChange={e => setTempBulletin(e.target.value)} className="w-full h-32 bg-white/5 border border-white/10 rounded-2xl p-4 text-sm outline-none text-white focus:border-cyan-500/50 resize-none font-bold" /><div className="flex gap-4"><button onClick={() => setIsBulletinOpen(false)} className="flex-1 py-4 text-white/40 text-xs font-bold uppercase tracking-widest uppercase">取消</button><button onClick={() => { setBulletin(tempBulletin); setIsBulletinOpen(false); setTempBulletin(""); logAction('發布公告', tempBulletin.substring(0,10)+'...'); }} className="flex-[2] py-4 bg-amber-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-xl btn-tactile uppercase">廣播發布</button></div></div>
        </div>
      )}
    </div>
  );

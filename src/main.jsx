...
const App = () => {
  ...
  const [matrix, setMatrix] = useState({ admin_rate_edit: false, empire_dial: true });
  ...
  const handleAddPersonnel = async () => {
    ...
    await setDoc(doc(db, "drivers", newDriver.phone), {
      name: newDriver.name, phone: newDriver.phone, pwd, timestamp: new Date(),
      delivered: 0, returned: 0, is_insured: newDriver.insured, insurance_base: newDriver.insured ? 30000 : 0
    });
    alert(`建檔成功，金鑰：${pwd}`);
  ...
  if (view === 'login') return (
    <div className="h-screen w-screen bg-black flex flex-col items-center justify-center p-8 text-white font-sans">
      ...
      <div className="w-full max-w-sm space-y-5 bg-white/5 border border-white/10 p-8 rounded-[3rem]">
        <input type="text" placeholder="手機" className="manual-input" onChange={e => setLoginInput({...loginInput, phone: e.target.value})} />
        <input type="password" placeholder="金鑰" className="manual-input tracking-widest" onChange={e => setLoginInput({...loginInput, pwd: e.target.value})} />
        <button onClick={handleLogin} className="w-full py-5 rounded-2xl bg-cyan-500 font-black uppercase tracking-widest shadow-xl">進入核心</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 pb-40 overflow-y-auto no-scrollbar">
      ...
      <header className="flex justify-between items-center mb-8 shrink-0">
        <div className="flex items-center gap-2"><Radio size={14} className="text-cyan-400 animate-pulse" /><span className="text-[10px] font-black uppercase tracking-widest text-white/40">Alpha Core v3.0 | 已連線</span></div>
      ...
      <main className="space-y-8">
        <div className="bg-zinc-900 border border-white/5 rounded-[2.5rem] p-10 flex items-center justify-between">
          <h2 className="text-3xl font-black italic">{role === 'sovereign' ? '指揮官' : role === 'admin' ? '稽核端' : `哨兵: ${activeSentinel?.name}`}</h2>
        ...
        {(role === 'sovereign' || role === 'admin') && (
           <div className={`bg-white/5 border border-cyan-500/20 rounded-[2.5rem] p-8 transition-all ${isAddingPersonnel ? 'h-auto' : 'h-24 overflow-hidden'}`}>
             <div className="flex justify-between items-center mb-6">
                <h3 className="text-xs font-black text-cyan-400 uppercase tracking-widest"><UserPlus className="inline mr-2" size={16}/> 人員錄入模組</h3>
                <button onClick={() => setIsAddingPersonnel(!isAddingPersonnel)}>{isAddingPersonnel ? <X/> : <Plus/>}</button>
             </div>
             {isAddingPersonnel && (
                <div className="space-y-4">
                  <input placeholder="姓名" className="manual-input !text-sm" onChange={e => setNewDriver({...newDriver, name: e.target.value})} />
                  <input placeholder="手機" className="manual-input !text-sm" onChange={e => setNewDriver({...newDriver, phone: e.target.value})} />
                  <button onClick={handleAddPersonnel} className="w-full py-4 bg-cyan-600 rounded-xl font-black uppercase">確認雲端同步</button>
                </div>
             )}
           </div>
        )}
        ...
        {role === 'sovereign' && (
          <div className="bg-zinc-900/50 border border-white/10 rounded-[2rem] p-8 space-y-4">
            <h3 className="text-xs font-black text-rose-500 uppercase flex items-center gap-2"><LockKeyhole size={16}/> 維度矩陣控制</h3>
            <div className="flex justify-between items-center p-4 bg-black rounded-2xl">
              <span className="text-xs font-bold">授權會計修改</span>
              <div onClick={async () => { const val = !matrix.admin_rate_edit; setMatrix({...matrix, admin_rate_edit: val}); await setDoc(doc(db, "system", "matrix"), {admin_rate_edit: val}); }} className={`switch-matrix ${matrix.admin_rate_edit ? 'active' : ''}`}><div className="knob"></div></div>
            </div>
          </div>
        )}
        ...
        {role === 'sentinel' && activeSentinel && (
          <div className="bg-zinc-900 p-12 rounded-[2.5rem] flex justify-center gap-8">
            <EmpireDial value={activeSentinel.delivered || 0} label="配送數" color="#22d3ee" onChange={() => {}} />
            <EmpireDial value={activeSentinel.returned || 0} label="回收數" color="#fbbf24" onChange={() => {}} />
          </div>
        )}
      </main>
